...
def eve_starter():
    ...
    for sysitr in range(totsys):
        ...
        if config[confsec].getboolean('enabled') is True:
            ...
            morbidostats.append([Morbidostat(sysnum, totsys, chips), sysnum])
            #Morbidostat(sysnum)
            # thread.join
        else:
...
class Morbidostat:
    def __init__(self, sysnum, totsys, chips):
        self.sysnum = sysnum
        self.totsys = totsys
        self.adc= chips['adc']
        ...
        self.time_between_pumps = config[self.sysstr].getfloat('time_between_pumps')
        self.OD_thr = config[self.sysstr].getfloat('OD_thr')
        self.OD_thr_set = False
        self.OD_min = config[self.sysstr].getfloat('OD_min')
        self.OD_err = config[self.sysstr].getfloat('OD_error')
        self.time_between_ODs = config[self.sysstr].getfloat('time_between_ODs') # how often to gather OD data, in seconds
        self.time_between_graphs = config[self.sysstr].getfloat('time_between_graphs') # how often to graph, in minutes
        ...
        self.time_between_saves = config[self.sysstr].getfloat('time_between_saves')

        # Set Up I2C to Read OD Data
        # Create the I2C bus

        self.P_drug_times = config[self.sysstr].getfloat('P_drug_times')
        self.P_nut_times = config[self.sysstr].getfloat('P_nut_times')
        self.P_waste_times = config[self.sysstr].getfloat('P_waste_times')
        ...
        self.avOD = 0
        self.OD_av_length = config[self.sysstr].getint('OD_av_length')
        # OD averaging buffer
        self.avOD_buffer = [0] * self.OD_av_length #need to change for multiplexing
        self.thresh_check = config[self.sysstr].getfloat('time_thresh')
        self.growthOD = []
        self.growthrate = []
        self.growthrate2 = []
        self.growthrate_t = []
        ...
        self.graph_loops = self.totsys * config['MAIN'].getint('graph_resolution_fac')
        ...
        self.total_time = config[self.sysstr].getfloat('Exp_time_hours')*3600 #in seconds
        ...
        self.photod = AnalogIn(self.adc[self.adc_add.index(config[self.sysstr].getint('a_address'))], getattr(ADS,'P'+ str(config[self.sysstr].getint('Analogin'))))

        # Setup the GPIO Pins to Control the Pumps
        self.pipins = config[self.sysstr].getboolean('Pi_pins')
        self.P_drug_pins = config[self.sysstr].getint('P_drug_pins')
        self.P_nut_pins = config[self.sysstr].getint('P_nut_pins')
        self.P_waste_pins = config[self.sysstr].getint('P_waste_pins')
        self.P_LED_pins = config[self.sysstr].getint('P_LED_pins')
	# P_fan_pins = config[self.sysstr].getint('P_fan_pins')
        ...
        if self.pipins:
            ...
            self.mcp = self.gpioe[self.gpio_add.index(config[self.sysstr].getint('m_address'))]
        ...
        self.slack_client = SlackClient(config['MAIN']['slack_key'])
        # self.chanid = config['MAIN']['slack_chanid']
        self.slack_usericon = config[self.sysstr]['slack_icon']
        self.chan = config['MAIN']['slack_channel']
